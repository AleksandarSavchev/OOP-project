...
Bank::Bank(const Bank & other)
{
	...
	{
			accounts.push_back(other.accounts[i]);
	}
}
...
Bank & Bank::operator=(const Bank & rhs)
{
	if (this != &rhs)
	{
		...
		{
			accounts.push_back(rhs.accounts[i]);
		}
	}
	return *this;
}
...
void Bank::addAccount(const char * accountType, const char * iban, const unsigned int ownerid, const int amount)
{
	...
	if (k < 0)
	{
		if (strcmp(accountType, "Savings Account") == 0)
		{
			double interestRate;
		...
		else if (strcmp(accountType, "Current Account") == 0)
		{
			accounts.push_back(new CurrentAccount(iban, ownerid, amount));
			std::cout << "Current Account added\n";
		}
		else if (strcmp(accountType, "Privilege Account") == 0)
		{
			int overdraft;
